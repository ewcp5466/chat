...
class AJAXChat {
	...
	function initRequestVars() {
		...
		if (false) {
			// It is safe to remove the slashes as we escape user data ourselves
			array_walk(
				$this->_requestVars,
				function (&$value, $key) {
					if (is_string($value)) {
	...
	function initChatViewSession() {
		...
		if($this->getChannel() !== null) {
			...
			if($this->getRequestVar('ajax')) {
				// Set channel, insert login messages and add to online list:
				$this->chatViewLogin();
			}
		}
	}
	...
	function isUserMentioned($text) {
		...
		if (empty($pun_user['username']) || !is_string($text)) {
			return false;
		}

		$username = preg_quote($pun_user['username'], '/');

		return preg_match(
			'/(^|[^A-Za-z0-9_])@'.$username.'(?=$|[^A-Za-z0-9_])/iu',
			$text
		) === 1;
	}
	...
	function insertParsedMessage($text) {
		...
		if(strpos($text, '/') === 0) {
			...
			switch($textParts[0]) {
				...
				case '/purge':
					$this->insertParsedMessagePurge($textParts);
					break;
	...
	function insertParsedMessagePurge($textParts) {
		// Only admins can purge
		if($this->getUserRole() != AJAX_CHAT_ADMIN) {
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				'/error CommandNotAllowed purge'
			);
			return;
		}

		$sql = 'DELETE FROM '.$this->getDataBaseTable('messages');
		$result = $this->db->sqlQuery($sql);
	...
	function getChatViewMessageXML(
		...
		$message .= '<mentioned>'.($this->isUserMentioned($text) ? '1' : '0').'</mentioned>';
		$message .= '</message>';
		return $message;
	}

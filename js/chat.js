...
var ajaxChat = {
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		if (this.userName && messageText) {
			var escapedUserName = this.escapeRegExp(this.userName);
			var mentionRegExp = new RegExp(
	...
	replaceCommands: function(text) {
		try {
			...
			switch(textParts[0]) {
				...
				case '/purge_exec':
					return this.replaceCommandPurgeExec();
				default:
	...
	replaceCommandLogin: function(textParts) {
		return	'<span class="chatBotMessage">' + this.lang['login'].replace(/%s/, textParts[1]) + '</span>';		
	},
	...
	replaceCommandLogout: function(textParts) {
		...
		if(textParts.length == 3)
			type = textParts[2];
		return	'<span class="chatBotMessage">' + this.lang['logout' + type].replace(/%s/, textParts[1]) + '</span>';		
	},
	
	replaceCommandChannelEnter: function(textParts) {
		return	'<span class="chatBotMessage">' + this.lang['channelEnter'].replace(/%s/, textParts[1]) + '</span>';		
	},
	
	replaceCommandChannelLeave: function(textParts) {
		return	'<span class="chatBotMessage">' + this.lang['channelLeave'].replace(/%s/, textParts[1]) + '</span>';		
	},
	...
	replaceCommandWhois: function(textParts) {
		return	'<span class="chatBotMessage">' + this.lang['whois'].replace(/%s/, textParts[1]) + ' ' + textParts[2] + '</span>';		
	},

	replaceCommandWhereis: function(textParts) {
		return	'<span class="chatBotMessage">' + this.lang['whereis'].replace(/%s/, textParts[1]).replace(
								/%s/,
								'<a href="javascript:ajaxChat.sendMessageWrapper(\'/join '
								+ this.scriptLinkEncode(textParts[2])
								+ '\');" title="'
	...
	replaceCommandRoll: function(textParts) {
		...
		return	'<span class="chatBotMessage">' + rollText + '</span>';		
	},
	...
	replaceCommandPurgeExec: function() {
		this.clearChatList();
		this.lastID = 0;
		return	'<span class="chatBotErrorMessage">Chat purged</span>';
	},
	...
	replaceBBCode: function(text) {
		if(!this.settings['bbCode']) {
			if (!arguments.callee.regExpStripBBCode) {
				arguments.callee.regExpStripBBCode = new RegExp('\\[(?:\\/)?(\\w+)(?:=([^<>]*?))?\\]', 'gm');
			}		
			return text.replace(arguments.callee.regExpStripBBCode, '');
		}
		if (!arguments.callee.regExp) {
			arguments.callee.regExp = new RegExp('\\[(\\w+)(?:=([^<>]*?))?\\](.+?)\\[\\/\\1\\]', 'gm');
		}		
		return text.replace(arguments.callee.regExp, this.replaceBBCodeCallback);
	},
	...
	replaceBBCodeUrl: function(content, attribute) {
		...
		if (!arguments.callee.regExpUrl) {
			arguments.callee.regExpUrl = new RegExp('^(?:(?:http)|(?:https)|(?:ftp)|(?:irc)):\\/\\/', '');
		}
		if(!url || !url.match(arguments.callee.regExpUrl))
			return content;
		return 	'<a href="' + url + '" onclick="window.open(this.href); return false;">' + this.replaceBBCode(content) + '</a>';
	},
	
	replaceBBCodeImage: function(url) {
		if(this.settings['bbCodeImages']) {
			if (!arguments.callee.regExpUrl) {
				arguments.callee.regExpUrl = new RegExp(this.regExpMediaUrl, '');
			}
			if(!url || !url.match(arguments.callee.regExpUrl))
				return url;
			...
			return	'<a href="' + url + '" onclick="window.open(this.href); return false;">'
					+ '<img class="bbCodeImage" style="max-width:' + maxWidth + 'px; max-height:' + maxHeight + 'px;" src="' + url + '" alt="" onload="ajaxChat.updateChatlistView();"/></a>';
		}
		return url;
	},
	...
	replaceBBCodeQuote: function(content, attribute) {
		if(attribute)
			return	'<span class="quote"><cite>' + this.lang['cite'].replace(/%s/, attribute) + '</cite><q>' + this.replaceBBCode(content) + '</q></span>';
	...
	replaceHyperLinks: function(text) {
		...
		if(!arguments.callee.regExp) {
			arguments.callee.regExp = new RegExp('(^|\\s|>)((?:(?:http)|(?:https)|(?:ftp)|(?:irc)):\\/\\/[^\\s<>]+)(<\\/a>)?', 'gm');
		}
		return text.replace(
			arguments.callee.regExp,
	...
	replaceEmoticonsCallback: function(str, p1, p2, p3) {
		...
		if(p2) {
			...
			return 	ajaxChat.replaceEmoticons(p1)
				+	'<img src="' + ajaxChat.dirs['emoticons'] + ajaxChat.emoticonFiles[index] + '" alt="' + p2 + '" />'
	...
	getCustomUserMenuItems: function(encodedUserName, userID) { return ''; },
	parseCustomInputMessage: function(text) { return text; },
	parseCustomInputCommand: function(text, textParts) { return text; },
	replaceCustomText: function(text) { return text; },
	replaceCustomCommands: function(text, textParts) { return text; },
